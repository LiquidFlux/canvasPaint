$(function(){
	...
	$('#navDesktop .brush').on('click', function(event){
		if( !$(this).parent(1).hasClass('current') ){
			$('#navDesktop .current').removeClass('current');
			$(this).parent(1).addClass('current');
			paint.setBrush($(this).data('brush-type'));
		}
	});
	$('#canvasButtonRestart').on('click', function(event){
		paint.clear();
	});
	$('#colorPicker').on('change', function(event){
		paint.setLineColor($(this).val());
	});
	$('#lineSizePicker').on('change', function(event){
		paint.setLineSize($(this).val());
	});
	paint = $('#mainCanvas').paint({
		aspectRatio: 2 / 1
	});
});
